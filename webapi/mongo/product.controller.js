...
async function getProductSpecial() {
  try {
    const result = await productModel.find({ "category.categoryName": "special" });
    return result;
  } catch (error) {
    console.log("Loi", error);
    throw error;
...
async function getProductDrink() {
  try {
    const result = await productModel.find({ "category.categoryName": "drink" });
    return result;
  } catch (error) {
    console.log("Loi", error);
    throw error;
...
async function searchProducts(keyword) {
  try {
    // Sử dụng biểu thức chính quy để tìm kiếm các sản phẩm có tên chứa từ khóa
...
async function updateById(id, body) {
  try {
    ...
    const result = await productModel.findByIdAndUpdate(
      id,
      { name, price, image, description, category: cateUpdate },
      { new: true }
    );
    return result;
  } catch (error) {
...
async function getRelatedProducts(currentProduct) {
  try {
    ...
    if (
      ...
      !currentProduct._id // Thêm kiểm tra này
    ) {
      throw new Error("Invalid current product data");
    }
      ...
      .find(
        {
          ...
          _id: { $ne: currentProduct._id && currentProduct._id }, // Thêm điều kiện này
        },
        { name: 1, price: 1, quantity: 1, category: 1, image: 1 }
      )
      .limit(3);
