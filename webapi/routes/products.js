...
const checktoken = require("../middleware/veryToken");
...
router.get("/special", async (req, res) => {
  try {
    const products = await productController.getProductSpecial();
    return res.status(200).json({ Products: products });
  } catch (error) {
    console.log("Loi");
    return res.status(500).json({ mess: error });
...
router.get("/drink", async (req, res) => {
  try {
    const products = await productController.getProductDrink();
    return res.status(200).json({ Products: products });
  } catch (error) {
    console.log("Loi");
    return res.status(500).json({ mess: error });
